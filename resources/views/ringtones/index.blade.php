    ...
    <div class="modal fade" id="modal{{preg_replace('/\s+/','',$page_title)}}" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="card-body">
                    <form id="form{{preg_replace('/\s+/','',$page_title)}}EditRingtone">
        ...
        $(function () {
            ...
            var dtTable = $('#table{{preg_replace('/\s+/','',$page_title)}}').DataTable({
                ...
                columns: [
                    ...
                    {data: 'ringtone_file',className: "text-center "},
                    {data: 'ringtone_name', className: "align-middle",},
                    {data: 'ringtone_view_count', className: "align-middle",},
                    {data: 'ringtone_like_count', className: "align-middle",},
                    {data: 'ringtone_extension', className: "align-middle",},
            ...
            $(document).on('click', '.edit{{preg_replace('/\s+/','',$page_title)}}', function (data) {
                ...
                $.ajax({
                    type: "get",
                    url: "{{ asset("admin/ringtones/edit") }}/" + id,
                    success: function (data) {
                        ...
                        $('#id').val(data.ringtone.id);


                        var id_tags =[];
                        $.each(data.ringtone.tags, function(i, item) {
            ...
            $('#form{{preg_replace('/\s+/','',$page_title)}}EditRingtone').on('submit', function (event) {
                event.preventDefault();
                var formData = new FormData($("#form{{preg_replace('/\s+/','',$page_title)}}EditRingtone")[0]);
                ...
                if ($('#saveBtn{{preg_replace('/\s+/','',$page_title)}}').val() == 'update') {
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.success) {
                                $('#form{{preg_replace('/\s+/','',$page_title)}}EditRingtone').trigger("reset");
                                toastr['success'](data.success, 'Success!');
