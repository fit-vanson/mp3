...
//        $searchTags = $columnName_arr[6]['search']['value'];
        ...
        $totalRecordswithFilter = Ringtones::select('count(*) as allcount')
            ->where('ringtone_name', 'like', '%' . $searchValue . '%')
            ->orwhereRelation('tags','tag_name','like', '%' . $searchValue . '%')
            ->count();

        // Get records, also we have included search filter as well
        $records = Ringtones::with('tags')
            ->where('ringtone_name', 'like', '%' . $searchValue . '%')
            ->orwhereRelation('tags','tag_name','like', '%' . $searchValue . '%')
            ->select('*')
...
//        }
        ...
        if ($request->get('null_tag') == 1){
            $totalRecordswithFilter = Ringtones::select('count(*) as allcount')
                ...
                ->where('ringtone_name', 'like', '%' . $searchValue . '%')
                ->count();
            $records = Ringtones::doesntHave('tags')
                ->where('ringtone_name', 'like', '%' . $searchValue . '%')
                ->select('*')
                ->orderBy($columnName, $columnSortOrder)
                ->skip($start)
...
//            $btn  = ' <a href="javascript:void(0)" onclick="editRolesPermissions('.$record->id.')" class="btn btn-warning"><i class="ti-pencil-alt"></i></a>';
            $btn = ' <a href="javascript:void(0)" data-id="'.$record->id.'" class="btn btn-warning editRingtones"><i class="ti-pencil-alt"></i></a>';
            $btn .= ' <a href="javascript:void(0)" data-id="'.$record->id.'" class="btn btn-danger deleteRingtones"><i class="ti-trash"></i></a>';
            $data_arr[] = array(
                "id" => $record->id,
//                "ringtone_file" => '<a class="image-popup-no-margins" href="'.url('/storage/ringtones').'/'.$record->ringtone_image.'"><img class="img-fluid" alt="'.$record->wallpaper_name.'" src="'.url('/storage/wallpapers/thumbnails').'/'.$record->wallpaper_image.'" width="75"></a>',
                "ringtone_file" => '<audio class="audio-player" controls><source src="'.url('/storage/ringtones').'/'.$record->ringtone_file.'" type="audio/mp3"/></audio>',
                "ringtone_name" => $record->ringtone_name,
                "ringtone_extension" => $record->ringtone_extension,
                "ringtone_view_count" => $record->ringtone_view_count,
                "ringtone_like_count" => $record->ringtone_like_count,
                "tags" => $record->tags,
                "action" => $btn,
            );
        }
    ...
    public function create(Request $request){
        ...
        $path_origin    =  storage_path('app/public/ringtones/'.$monthYear.'/');
...
//            $img->save($path.'thumbnails/'.$wallpaper->wallpaper_image);
    ...
    public function compare(){
        ...
        if (isset($wallpaper_check)){
            ...
            echo 'Success';
            Log::info('Message: Wallpaper_status = 0 null -- success ');
        }
